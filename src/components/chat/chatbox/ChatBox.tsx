...
const ChatBox = ({
  ...
  socketContext.socket?.on("receiveMessage", (data: any) => {
    const isCurrentUserChat =
      ref.current?._id === data.senderId ||
      ref.current?.user_id === data.senderId;
  ...
  const handleSendMessage = () => {
    socketContext.socket?.emit("sendMessage", {
      ...
      receiverId: selectedUser?._id || selectedUser?.user_id,
      message,
    });
    setMessage("");
    setMessageThread([
      ...messageThread,
      { senderId: userData?._id, receiverId: selectedUser?._id, message },
    ]);
    getUserList();
  };
      ...
      <div className="max-h-full overflow-y-auto py-4 md:py-5 px-2.5 xl:px-7">
        <div className="w-full flex justify-center items-center mb-5">
          <span className="text-black text-[10px] md:text-xs font-normal leading-3 md:leading-4 bg-light-gray-300  rounded-[14px] px-3 xl:px-6 py-3 xl:py-5 max-w-full lg:max-w-[568px] w-[568px] flex flex-row items-center gap-2.5">
            <img
              ...
              height={21}
              width={18}
              className="h-[16px] w-[14px] md:w-[18px] md:h-[21px] "
            />{" "}
        ...
        <div className="flex flex-col w-full gap-5 px-0 xl:px-5">
          {messageThread.map((item, index) => (
            <div
              key={index}
              className={`rounded-t-[20px] text-white p-3 text-sm font-normal leading-4 ${
      ...
      <div className="w-full py-3 px-3 xl:px-7">
        <div className="flex flex-row gap-3 py-1.5 pl-4 pr-2 items-center bg-white border border-gray-400 rounded-full ">
          ...
          <button
            ...
            className="bg-black text-white px-3 xl:px-5 py-1 xl:py-2 rounded-full text-sm xl:text-base font-normal flex flex-row items-center gap-1.5"
